...
| 역할 | 어디서 도는가 | 설명 |
| --- | --- | --- |
| 자동 실행(시계) + 파이프라인 | **GitHub Actions** | 하루 3회 실행. 조사·작성·검증·게시를 모두 처리합니다 |
...
> 허용됩니다. 하루 3회를 무료로 돌리려면 GitHub Actions가 가장 확실합니다.
...
그 카테고리를 우선합니다. 조사가 실패하면 숫자에 의존하지 않는 **부동산상식** 글로 대체합니다.
...
하루 1회 종합 조사 + 3건 작성·검증 기준으로 **월 $10~20** 정도를 예상합니다.
